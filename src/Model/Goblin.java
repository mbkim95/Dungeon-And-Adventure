...
public class Goblin extends Monster {
	...
	public Goblin(Controller gameScreen) {
	...
	public void draw(Graphics2D g, BufferedImage img) {
		if (status == 0) {
			int sx = (frameList.get(nowFrame)) * 47;
			int sy = (frameList.get(nowFrame));
			gameScreen.drawImageClip(g, img, (int) x, (int) y, sx, sy, 47, 76, 7);
		} else if (status == 1) {
			...
			int sx = (frameList.get(nowFrame) % 4) * 100;
			int sy = (frameList.get(nowFrame) / 4);
			gameScreen.drawImageClip(g, img, (int) x, (int) y, sx, sy, 100, 113, 7);
		} else if (status == 3) {
