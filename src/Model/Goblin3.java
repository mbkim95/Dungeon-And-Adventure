...
public class Goblin3 extends Monster {
	public Goblin3(Controller gameScreen) {
	...
	public void draw(Graphics2D g, BufferedImage img) {
		if (status == 0) {
			int sx = (frameList.get(nowFrame)) * 55;
			int sy = (frameList.get(nowFrame));
			gameScreen.drawImageClip(g, img, (int) x, (int) y, sx, sy, 55, 86, 7);
		} else if (status == 1) {
			int sx = (frameList.get(nowFrame) % 6) * 61;
			int sy = (frameList.get(nowFrame) / 6);
			gameScreen.drawImageClip(g, img, (int) x, (int) y, sx, sy, 61, 86, 7);
		} else if (status == 2) {
			int sx = (frameList.get(nowFrame) % 4) * 120;
			int sy = (frameList.get(nowFrame) / 4);
			gameScreen.drawImageClip(g, img, (int) x, (int) y, sx, sy, 120, 120, 7);
		} else if (status == 3) {
			int sx = (frameList.get(nowFrame) % 5) * 97;
			int sy = (frameList.get(nowFrame) / 5) * 81;
			gameScreen.drawImageClip(g, img, (int) x, (int) y, sx, sy, 97, 81, 7);
		}
	}
